...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("CLUES.md"),
            dir.join("CLUES.local.md"),
            dir.join(".clues").join("CLUES.md"),
            dir.join(".clues").join("instructions.md"),
        ] {
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No project settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".clues")).expect("nested clues dir");
        fs::write(root.join("CLUES.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("CLUES.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".clues")).expect("apps clues dir");
        fs::write(root.join("apps").join("CLUES.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".clues").join("instructions.md"),
            "apps dot clues instructions",
        )
        .expect("write apps dot clues instructions");
        fs::write(nested.join(".clues").join("CLUES.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".clues").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("CLUES.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("CLUES.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.clues/CLUES.md")),
            "CLUES.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("CLUES.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? CLUES.md"));
    ...
    #[test]
    fn load_system_prompt_reads_clues_files_and_config() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".clues")).expect("clues dir");
        fs::write(root.join("CLUES.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".clues").join("settings.json"),
        ...
        let original_clues_home = std::env::var("CLUES_CONFIG_HOME").ok();
        std::env::set_var("HOME", &root);
        std::env::set_var("CLUES_CONFIG_HOME", root.join("missing-home"));
...
",
        ...
        if let Some(value) = original_clues_home {
            std::env::set_var("CLUES_CONFIG_HOME", value);
        } else {
    ...
    #[test]
    fn renders_project_style_sections_with_project_context() {
        let root = temp_dir();
        fs::create_dir_all(root.join(".clues")).expect("clues dir");
        fs::write(root.join("CLUES.md"), "Project rules").expect("write CLUES.md");
        fs::write(
            root.join(".clues").join("settings.json"),
            r#"{"permissionMode":"acceptEdits"}"#,
        )
    ...
    #[test]
    fn discovers_dot_clues_instructions_markdown() {
        let root = temp_dir();
        let nested = root.join("apps").join("api");
        fs::create_dir_all(nested.join(".clues")).expect("nested clues dir");
        fs::write(
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".clues/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/CLUES.md"),
